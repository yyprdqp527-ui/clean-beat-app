...
def cron_debug_reminder():
    ...
    try:
        ...
        row = c.fetchone()
        if not row or not row[0]:
            return jsonify({'error': 'user not found or no house_id', 'email': email}), 404
        house_id = row[0]
        paris_today_str = now_paris().strftime('%Y-%m-%d')
        c.execute("SELECT CURRENT_TIMESTAMP")
        db_now = str(c.fetchone()[0])
        c.execute("""
            SELECT m.id, m.content, m.timestamp, m.recipient_email,
                   EXISTS(SELECT 1 FROM message_reads mr WHERE mr.message_id = m.id AND mr.user_email = ?) AS is_read
            FROM messages m
            WHERE m.house_id = ?
              AND m.message_type = 'reminder'
        ...
        """, (email, house_id, email))
        recent = [
            {'id': r[0], 'content': (r[1] or '')[:80], 'timestamp': str(r[2]),
             'recipient_email': r[3], 'is_read': bool(r[4])}
            for r in c.fetchall()
        ]
        c.execute("""
            SELECT m.id, m.content, m.timestamp FROM messages m
            ...
            ORDER BY m.timestamp DESC LIMIT 1
        """, (house_id, email, paris_today_str, email))
        cas2 = c.fetchone()
        cas2_result = {'id': cas2[0], 'content': (cas2[1] or '')[:80], 'timestamp': str(cas2[2])} if cas2 else None
        c.execute("""
            SELECT m.id, m.content, m.timestamp FROM messages m
            WHERE m.house_id = ? AND m.message_type = 'reminder'
              AND (m.recipient_email = ? OR m.recipient_email IS NULL)
              AND NOT EXISTS (SELECT 1 FROM message_reads mr WHERE mr.message_id = m.id AND mr.user_email = ?)
            ORDER BY m.timestamp DESC LIMIT 1
        """, (house_id, email, email))
        latest = c.fetchone()
        latest_result = {'id': latest[0], 'content': (latest[1] or '')[:80], 'timestamp': str(latest[2])} if latest else None
        conn.close()
        return jsonify({
            'email': email,
            'house_id': house_id,
            'now_paris_python': now_paris().strftime('%Y-%m-%d %H:%M:%S'),
            'paris_today_str': paris_today_str,
            'db_current_timestamp': db_now,
            'recent_reminders': recent,
            'menu_cas2_fallback_today': cas2_result,
            'api_latest_reminder': latest_result,
        })
    except Exception as e:
        import traceback
